...
class OpenregisterClient(BaseClient):
    ...
    def search_companies(self, company_name: str = None, 
        ...
        if max(result) > 75:
            self.logger.debug(f"Found a matching company")
            return result[max(result)]
        self.logger.debug(f"Didn't find a matching company")
        return {}
    ...
    def enrich_data(self, known_data: CompanyData) -> CompanyData:
        ...
        if search_required:     # Search for the company to get it's company_id
            ...
            if not company:
                self.logger.debug(f"Could not find a mathcing company")
                return known_data
            company_id = company["company_id"]

        self.logger.debug(f"Found company, mapping information")
